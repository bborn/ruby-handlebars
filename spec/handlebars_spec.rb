...
describe Handlebars::Handlebars do
  ...
  context "evaluating" do
    ...
    it "a simple replacement with stripped leading whitespace" do
      expect(evaluate("foo{{~var}}", {var: " bar"})).to eq("foobar")
    end

    it "a simple replacement with stripped trailing whitespace" do
      expect(evaluate("{{var~}}bar", {var: "foo "})).to eq("foobar")
    end

    it "a simple replacement with stripped leading and trailing whitespace" do
      expect(evaluate("foo{{~var~}}baz", {var: " bar "})).to eq("foobarbaz")
    end
    ...
    it "a triple braces replacement with whitespace control" do
      expect(evaluate("Hello{{{~name~}}}", {name: ' <"\'>& '})).to eq('Hello<"\'>&')
    end
    ...
    context "helpers" do
      ...
      it "with an empty string argument" do
        hbs.register_helper("noah") { |context, value| value.to_s.delete("a") }
